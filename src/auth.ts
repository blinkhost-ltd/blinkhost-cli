...
function errorCode(data: unknown): string {
  return data && typeof data === 'object' && typeof (data as { error?: unknown }).error === 'string' ? (data as { error: string }).error : '';
}

export async function login(options: { profile?: string; apiOrigin?: string; openBrowser?: boolean; progress?: (line: string) => void }): Promise<unknown> {
  ...
  if (!started.response.ok) throw new CliError('BlinkHost could not start CLI authorization.', EXIT.auth, errorCode(started.data) || 'authorization_start_failed');
  const device = started.data as { device_code: string; user_code: string; verification_uri_complete: string; expires_in: number; interval: number };
  options.progress?.(`Open ${device.verification_uri_complete}`);
  options.progress?.(`Confirm code ${device.user_code}`);
  if (options.openBrowser !== false) openBrowser(device.verification_uri_complete);
  ...
  while (Date.now() < deadline) {
    ...
    if (!polled.response.ok) throw new CliError(code === 'access_denied' ? 'CLI authorization was denied.' : 'CLI authorization could not be completed.', EXIT.auth, code || 'authorization_failed');
    const tokens = polled.data as { access_token: string; refresh_token: string };
    ...
    const capabilities = await client.request('/api/cli/v2/capabilities/') as { actor?: { id?: number; username?: string } };
    await client.rememberIdentity(capabilities.actor || {});
    return { profile: profileName, api_origin: apiOrigin, account: capabilities.actor, permissions: (capabilities as { features?: unknown }).features };
  }
  throw new CliError('CLI authorization expired before it was approved.', EXIT.auth, 'authorization_expired');
}

export async function logout(profile?: string): Promise<{ profile: string }> {
  ...
  try {
    ...
    if (!(error instanceof CliError) || error.code !== 'session_expired') throw error;
  }
