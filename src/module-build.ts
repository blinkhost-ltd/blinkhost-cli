...
const uuid = (value: unknown) => typeof value === 'string' && /^[a-f0-9]{8}(?:-[a-f0-9]{4}){3}-[a-f0-9]{12}$/i.exec(value)?.[0] === value;
const hash = (value: unknown) => typeof value === 'string' && /^[a-f0-9]{64}$/.exec(value)?.[0] === value;
...
export function validateModuleBuild(value: unknown, expected: {
  project: string; review?: string; module?: string; revision?: string; digest?: string;
}, action: string): void {
  ...
  if (action === 'build-modules') {
    if (!Array.isArray(item.modules) || item.modules.length > 1000 || item.modules.some(module => !module
      ...
      || new Set(item.modules.map(module => module.id)).size !== item.modules.length) return fail();
    return;
  }
  const usage = item.usage as Record<string, unknown> | null;
  if (!uuid(item.id) || !uuid(item.module_id) || !hash(item.source_digest) || !hash(item.action_digest)
    ...
    || (action === 'build-approve' && !item.applied_at)) return fail();
}
