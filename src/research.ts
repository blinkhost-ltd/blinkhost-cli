...
export function validateResearchCatalog(raw: unknown, project: string): void {
  const fail = (): never => { throw new CliError('The public source catalog could not be verified. No research was started.', EXIT.remote, 'research_catalog_invalid'); };
  const text = (value: unknown, max: number, multiline = false): value is string => typeof value === 'string'
  ...
  if (!value || value.project_id !== project.toLowerCase() || value.live_search !== false
    ...
    || !Array.isArray(value.sources) || value.sources.length > 100) fail();
  const seen = new Set<string>();
  for (const source of value!.sources as Record<string, unknown>[]) {
    if (!source || typeof source !== 'object' || Array.isArray(source)
      ...
      || !/^https:\/\/[a-z0-9.-]+\/[A-Za-z0-9_./~%\-]*$/.test(source.url)) fail();
    let url: URL;
    try { url = new URL(source.url as string); } catch { return fail(); }
    if (!/\.[a-z]{2,63}$/.test(url.hostname) || /\.(localhost|local|internal|test|invalid)$/.test(url.hostname)
      ...
      || /%(?![0-9a-f]{2})|%(0[0-9a-f]|1[0-9a-f]|7f|25|2f|5c)/i.test(url.pathname)) fail();
    ...
    if (!Number.isFinite(captured) || !Number.isFinite(expires) || expires <= captured || expires - captured > 90 * 86400000) fail();
