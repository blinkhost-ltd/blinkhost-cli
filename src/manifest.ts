...
export interface BlinkHostManifest {
  ...
  resources: {
    databases: Array<{ binding: string; schema?: string; migrations?: string; seeds?: string }>;
    secrets: string[];
  };
  preview: { enabled: boolean; database_mode: 'none' | 'isolated_branch' };
  ignore: string[];
}
...
function object(value: unknown, field: string, allowed: Set<string>): Record<string, unknown> {
  if (!value || typeof value !== 'object' || Array.isArray(value)) fail(field, 'Must be an object.');
  const result = value as Record<string, unknown>;
  const unknown = Object.keys(result).filter((key) => !allowed.has(key)).sort();
  if (unknown.length) fail(field, `Unsupported fields: ${unknown.join(', ')}.`);
  return result;
}

function stringValue(value: unknown, field: string, maximum = 512, required = true): string {
  if (value === undefined || value === null) {
    if (!required) return '';
    fail(field, 'Must be a non-empty string.');
  }
  if (typeof value !== 'string' || (required && !value.trim())) fail(field, 'Must be a non-empty string.');
  const normalized = value.trim();
  if (Buffer.byteLength(normalized, 'utf8') > maximum) fail(field, `Must not exceed ${maximum} UTF-8 bytes.`);
  if (/[\u0000-\u001f\u007f]/.test(normalized)) fail(field, 'Control characters are not supported.');
  return normalized;
}

export function normalizeRepositoryPath(value: unknown, field: string, allowRoot = false): string {
  const path = stringValue(value, field, 1024);
  if (path === '.' && allowRoot) return path;
  if (path.startsWith('/') || path.startsWith('\\') || path.includes('\\') || path.includes('\0')) fail(field, 'Must be a repository-relative POSIX path.');
  if (path.endsWith('/') || path === '.' || path === '..' || path.includes('//')) fail(field, 'Must be a normalized repository-relative path.');
  const parts = path.split('/');
  if (parts.length > 64) fail(field, 'Must not exceed 64 path segments.');
  for (const part of parts) {
    const folded = part.toLowerCase();
    const stem = folded.split('.', 1)[0] ?? folded;
    if (!part || part === '.' || part === '..' || folded === '.git' || WINDOWS_NAMES.has(stem)) fail(field, `Contains reserved path segment: ${part}.`);
    if (part.endsWith(' ') || part.endsWith('.')) fail(field, 'Path segments cannot end with a space or period.');
    if (/[\u0000-\u001f\u007f]/.test(part)) fail(field, 'Path contains control characters.');
  }
  return parts.join('/');
}

function command(value: unknown, field: string, required = true): string {
  const result = stringValue(value, field, 1024, required);
  if (/[\r\n]/.test(result)) fail(field, 'Build commands must be single-line values.');
  return result;
}

export function parseManifest(raw: string | Uint8Array): BlinkHostManifest {
  const bytes = typeof raw === 'string' ? Buffer.from(raw, 'utf8') : Buffer.from(raw);
  if (bytes.byteLength > MAX_MANIFEST_BYTES) fail('manifest', `Manifest exceeds ${MAX_MANIFEST_BYTES} bytes.`);
  const text = bytes.toString('utf8');
  if (!Buffer.from(text, 'utf8').equals(bytes)) fail('manifest', 'Manifest must be valid UTF-8.');
  const document = parseDocument(text, { strict: true, uniqueKeys: true });
  let hasAlias = false;
  visit(document, { Alias: () => { hasAlias = true; } });
  if (hasAlias) fail('manifest', 'YAML aliases are not supported.');
  if (document.errors.length) fail('manifest', `Manifest YAML is invalid: ${document.errors[0]?.message ?? 'unknown error'}`);
  const rootObject = object(document.toJS({ maxAliasCount: 0 }), 'manifest', TOP_LEVEL);
  if (rootObject.schema !== MANIFEST_SCHEMA) fail('schema', `Only ${MANIFEST_SCHEMA} is supported.`);
  ...
  if (dependencyRoot !== '.' && frontendRoot !== dependencyRoot && !frontendRoot.startsWith(`${dependencyRoot}/`)) fail('frontend.dependency_root', 'Must be the frontend directory or one of its parent directories.');
  const framework = stringValue(frontend.framework, 'frontend.framework', 32).toLowerCase();
  if (!SUPPORTED_FRONTENDS.includes(framework as FrontendFramework)) fail('frontend.framework', 'Unsupported frontend framework.');
  const packageManager = stringValue(frontend.package_manager, 'frontend.package_manager', 16).toLowerCase();
  if (!SUPPORTED_MANAGERS.includes(packageManager as PackageManager)) fail('frontend.package_manager', 'Unsupported package manager.');
  const normalizedFrontend: BlinkHostManifest['frontend'] = {
    root: frontendRoot,
    dependency_root: dependencyRoot,
    framework: framework as FrontendFramework,
    package_manager: packageManager as PackageManager,
  ...
  if (!Array.isArray(rawModules) || rawModules.length > 100) fail('modules', 'Must be an array containing at most 100 modules.');
  ...
  const modules = rawModules.map((rawModule, index) => {
    ...
    if (!MODULE_NAME.test(name)) fail(`${field}.name`, 'Use lowercase letters, numbers and internal hyphens.');
    ...
    return {
      name,
      path,
      language: language as ModuleLanguage,
  ...
  if (!Array.isArray(rawDatabases) || rawDatabases.length > 32) fail('resources.databases', 'Must be an array containing at most 32 bindings.');
  ...
  const databases = rawDatabases.map((rawDatabase, index) => {
    ...
    if (!BINDING_NAME.test(binding) || bindings.has(binding)) fail(`${field}.binding`, 'Bindings must be unique uppercase environment names.');
    bindings.add(binding);
    const result: { binding: string; schema?: string; migrations?: string; seeds?: string } = { binding };
    for (const key of ['schema', 'migrations', 'seeds'] as const) if (database[key] !== undefined) result[key] = normalizeRepositoryPath(database[key], `${field}.${key}`);
    return result;
  });
  const rawSecrets = resources.secrets ?? [];
  if (!Array.isArray(rawSecrets) || rawSecrets.length > 256) fail('resources.secrets', 'Must be an array containing at most 256 names.');
  const secrets = rawSecrets.map((value, index) => stringValue(value, `resources.secrets.${index}`, 128));
  if (new Set(secrets).size !== secrets.length || secrets.some((secret) => !BINDING_NAME.test(secret))) fail('resources.secrets', 'Secret names must be unique uppercase environment names.');

  const preview = object(rootObject.preview ?? {}, 'preview', PREVIEW_FIELDS);
  const enabled = preview.enabled ?? true;
  if (typeof enabled !== 'boolean') fail('preview.enabled', 'Must be a boolean.');
  const databaseMode = stringValue(preview.database_mode ?? 'none', 'preview.database_mode', 32).toLowerCase();
  if (databaseMode !== 'none' && databaseMode !== 'isolated_branch') fail('preview.database_mode', 'Unsupported preview database mode.');
  const rawIgnore = rootObject.ignore ?? [];
  if (!Array.isArray(rawIgnore) || rawIgnore.length > 256) fail('ignore', 'Must be an array containing at most 256 paths.');
...
export function serializeManifest(manifest: BlinkHostManifest): string {
