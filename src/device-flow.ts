...
type DevicePolling = { device_code: string; expires_in: number; interval: number };
type DeviceAuthorization = DevicePolling & { user_code: string; verification_uri_complete: string };
...
const services: PollingServices = {
...
function expired(): never {
  throw new CliError('This sign-in code has expired. Run `blinkhost auth login` again to get a new code.',
    EXIT.auth, 'authorization_expired');
}

function validatePolling(device: DevicePolling): void {
  if (!device || typeof device.device_code !== 'string' || !device.device_code || device.device_code.length > 4096
    ...
    throw new CliError('BlinkHost returned an invalid sign-in request. No authorization checks were sent.',
      EXIT.auth, 'authorization_response_invalid');
  }
}

/** Validate the server contract before displaying terminal text or launching a browser. */
export function validateDeviceAuthorization(data: unknown): DeviceAuthorization {
  if (!data || typeof data !== 'object' || Array.isArray(data)) {
    throw new CliError('BlinkHost returned an invalid sign-in request.', EXIT.auth, 'authorization_response_invalid');
  }
  const device = data as DeviceAuthorization;
  ...
  if (typeof device.user_code !== 'string' || device.user_code.length !== 9 || !/^[A-Z2-9]{4}-[A-Z2-9]{4}$/.test(device.user_code)
    ...
    throw new CliError('BlinkHost returned an invalid sign-in address or code. No browser was opened.',
      EXIT.auth, 'authorization_response_invalid');
  }
  return device;
}

export function validateDeviceTokens(data: unknown): { access_token: string; refresh_token: string } {
  ...
  if (!tokens || typeof tokens !== 'object' || Array.isArray(tokens)
    ...
    throw new CliError('BlinkHost returned an invalid sign-in session. No new credential was saved.',
      EXIT.auth, 'authorization_response_invalid');
  }
...
export async function pollDeviceAuthorization(apiOrigin: string, device: DevicePolling, verifier: string,
  dependencies: PollingServices = services): Promise<unknown> {
  validatePolling(device);
  ...
  while (dependencies.now() < deadline) {
    ...
    if (dependencies.now() >= deadline) return expired();
    ...
    if (!polled.response.ok) {
      ...
      if (code === 'access_denied') throw new CliError('CLI authorization was denied. No new session was saved.',
        EXIT.auth, 'access_denied');
      throw new CliError('CLI authorization could not be completed. Run `blinkhost auth login` to start a new sign-in.',
        EXIT.auth, 'authorization_failed');
    }
    return data;
  }
  return expired();
