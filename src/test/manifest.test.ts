...
test('every supported frontend and backend language produces the v1 contract', () => {
  for (const framework of ['astro', 'html', 'react', 'solid', 'svelte', 'vue'] as const) {
    for (const language of ['go', 'javascript', 'python', 'rust', 'typescript'] as const) {
