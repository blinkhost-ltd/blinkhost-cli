...
test('missing credential tooling stops login before device authorization or browser progress', async () => {
  ...
  try {
    await assert.rejects(login({ openBrowser: false, scopes: ['ai:read'], progress: line => progress.push(line) }),
      (error: unknown) => error instanceof CliError && error.code === 'credential_store_unavailable');
