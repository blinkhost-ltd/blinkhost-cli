...
function harness(responses: Array<{ status: number; data: unknown } | Error>) {
  let time = 0;
  let calls = 0;
  const waits: number[] = [];
  return {
    ...
    services: {
      now: () => time,
      sleep: async (milliseconds: number) => { waits.push(milliseconds); time += milliseconds; },
      request: async (_origin: string, path: string, init?: RequestInit) => {
...
const isError = (code: string) => (error: unknown) => error instanceof CliError && error.code === code;
...
test('server expiry and denial give actionable errors without retrying', async () => {
  for (const [remote, expected] of [['expired_token', 'authorization_expired'], ['access_denied', 'access_denied']] as const) {
