...
export function terminalJson(serialized: string): string {
  return serialized.replace(/[\p{Cf}\p{Zl}\p{Zp}\u007f-\u009f]/gu, char =>
    char.split('').map(unit => `\\u${unit.charCodeAt(0).toString(16).padStart(4, '0')}`).join(''));
}
