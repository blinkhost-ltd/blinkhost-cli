...
test('release checks and notes use the public BlinkHost registry without installing', async () => {
    ...
    globalThis.fetch = async (input) => {
        ...
        if (String(input).includes('/latest/'))
            return new Response(JSON.stringify({ current_version: '2.4.0', latest_version: '2.5.0', update_available: true, release_url: 'https://app.blinkhost.me/docs/releases/cli/2.5.0', install_command: 'npm install --global @blinkhost/cli@2.5.0' }), { status: 200, headers: { 'content-type': 'application/json' } });
    ...
    try {
        ...
        const notes = await releaseNotes('2.5.0');
...
test('function invocation uses the dedicated API with bounded explicit idempotency', async () => {
    ...
    try {
        ...
        await assert.rejects(runFunctions([
            'invocations', 'retry',
            '6b1f7131-f314-4f10-b8b8-8e59a414fc4c',
        ...
        if (previousToken === undefined)
            ...
            process.env.BLINKHOST_ACCESS_TOKEN = previousToken;
    }
});
test('function status uses the project-scoped capability endpoint', async () => {
    ...
    try {
        ...
        assert.equal(capturedUrl, 'https://api.blinkhost.me/api/backend-modules/rollout-status/?site_id=2a594fd8-cc93-47c1-864f-697765d5bd23');
    }
    finally {
        globalThis.fetch = previousFetch;
        if (previousToken === undefined)
