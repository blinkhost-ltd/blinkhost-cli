        ...
        for (const mode of [undefined, 'portable', 'SOURCE', 'source, portable', 'source']) {
            ...
            globalThis.fetch = async (url) => {
                ...
                return new Response(new ReadableStream({
                    start(controller) { controller.enqueue(zip); if (mode === 'source')
                        controller.close(); },
                    cancel() { cancelled = true; },
                }), { headers: { 'content-type': 'application/zip', ...(mode ? { 'x-blinkhost-export-mode': mode } : {}) } });
            };
            if (mode === 'source')
                assert.deepEqual(await client().projectArchive(project, 'source'), zip);
            else {
                await assert.rejects(client().projectArchive(project, 'source'), { code: 'export_mode_unconfirmed' });
                assert.equal(cancelled, true);
            }
        }
        await assert.rejects(client().projectArchive(project, 'unknown'), { code: 'export_mode_invalid' });
...
test('source-only command accepts either flag order, explains limitations and rejects repeated flags', async () => {
    ...
    try {
        for (const [index, flags] of [['first', ['--source-only', '--output']], ['last', ['--output']]]) {
...
//# sourceMappingURL=project-export.test.js.map