...
import { starterName, starterLanguage, validateStarter } from './starter.js';
...
function option(args, flag) {
    ...
    if (!value || value.startsWith('--'))
        throw new CliError(`${flag} needs a value.`, EXIT.usage, 'missing_option_value');
    args.splice(index, 2);
    return value;
}
function identifier(value) {
    if (!value || !uuid.test(value))
        throw new CliError('Provide a valid project or task UUID.', EXIT.usage, 'invalid_identifier');
    return value;
}
function noExtra(args) {
    if (args.length)
        throw new CliError('Unexpected assistant argument. Use blinkhost help ai.', EXIT.usage, 'unexpected_argument');
}
async function readRequestObject(path) {
    if (!path?.startsWith('@') || path.length < 2)
        throw new CliError('Use --request @request.json. Do not put prompts or credentials in command arguments.', EXIT.usage, 'request_file_required');
    ...
    try {
        ...
        if (!metadata.isFile() || metadata.size > 16384)
            throw new CliError('Use a regular request file no larger than 16 KiB.', EXIT.validation, 'request_file_invalid');
        const buffer = Buffer.alloc(16385);
        const { bytesRead } = await handle.read(buffer, 0, buffer.length, 0);
        if (bytesRead > 16384)
            throw new CliError('The request file exceeds 16 KiB.', EXIT.validation, 'request_file_invalid');
        let value;
        try {
            value = JSON.parse(new TextDecoder('utf-8', { fatal: true }).decode(buffer.subarray(0, bytesRead)));
        }
        catch {
            throw new CliError('The request file must contain a UTF-8 JSON object.', EXIT.validation, 'request_invalid');
        }
        if (!value || typeof value !== 'object' || Array.isArray(value))
            throw new CliError('The request must be a JSON object.', EXIT.validation, 'request_invalid');
        return value;
    }
    finally {
        await handle.close();
...
export async function readAssistantRequest(path) {
...
export async function readWorkflowPlan(path) {
    const body = await readRequestObject(path);
...
export async function readProjectGuidance(path) {
    if (!path?.startsWith('@') || path.length < 2)
        throw new CliError('Use --file @guidance.txt. Do not put guidance or credentials in command arguments.', EXIT.usage, 'guidance_file_required');
    const handle = await open(path.slice(1), constants.O_RDONLY | constants.O_NOFOLLOW | constants.O_NONBLOCK).catch(() => {
    ...
    try {
        ...
        if (!metadata.isFile() || metadata.size > 4096)
            throw new CliError('Use a regular UTF-8 text file no larger than 4 KiB.', EXIT.validation, 'guidance_file_invalid');
        const buffer = Buffer.alloc(4097);
        const { bytesRead } = await handle.read(buffer, 0, buffer.length, 0);
        if (bytesRead > 4096)
            throw new CliError('The guidance file exceeds 4 KiB.', EXIT.validation, 'guidance_file_invalid');
        let content;
        try {
            content = new TextDecoder('utf-8', { fatal: true }).decode(buffer.subarray(0, bytesRead));
        }
        catch {
            throw new CliError('The guidance file must be valid UTF-8 text.', EXIT.validation, 'guidance_file_invalid');
        }
        if (!content.trim() || /[\x00-\x08\x0b\x0c\x0e-\x1f]/.test(content))
            throw new CliError('Use nonempty text without control characters. Use memory-clear to remove saved guidance.', EXIT.validation, 'guidance_file_invalid');
        return content;
    }
    finally {
        await handle.close();
...
export async function runAssistant(input, profile) {
    const args = [...input];
    ...
    if (['starters', 'starter-review', 'starter-status', 'starter-approve'].includes(action ?? '')) {
        ...
        if (action === 'starters') {
            ...
            if (action === 'starter-approve') {
                if (option(args, '--confirm') !== review)
                    throw new CliError('Repeat the exact review UUID with --confirm to save the reviewed starter.', EXIT.usage, 'confirmation_required');
                const value = option(args, '--digest');
                if (!value || !digest.test(value))
                    throw new CliError('Pass --digest from the starter review you inspected.', EXIT.usage, 'review_digest_required');
                path += 'apply/';
                ...
                message = 'Starter review and save receipt loaded. A historical receipt does not establish current source, build or deployment health.';
        }
        noExtra(args);
    }
        ...
        if (action === 'build-modules') {
            ...
            const revision = option(args, '--revision');
            if (!revision || !digest.test(revision))
                throw new CliError('Use --revision from build-modules for the saved source you inspected.', EXIT.usage, 'source_revision_required');
            const requestId = identifier(option(args, '--request-id')).toLowerCase();
            path += 'review/';
            ...
            if (action === 'build-approve') {
                if (option(args, '--confirm') !== review)
                    throw new CliError('Repeat the exact review UUID with --confirm to request one metered build.', EXIT.usage, 'confirmation_required');
                const value = option(args, '--digest');
                if (!value || !digest.test(value))
                    throw new CliError('Pass --digest from the build review you inspected.', EXIT.usage, 'review_digest_required');
                path += 'apply/';
                body = { action_digest: value };
                ...
                message = 'Build review and receipt loaded. A receipt is not proof of successful compilation or deployment.';
        }
        noExtra(args);
    }
        ...
        if (action === 'memory-show') {
            ...
            if (option(args, '--confirm') !== project)
                throw new CliError('Repeat the project UUID with --confirm to change your guidance for future tasks.', EXIT.usage, 'confirmation_required');
        ...
        if (action === 'workflow-status') {
            ...
            if (option(args, '--confirm') !== id)
                throw new CliError('Repeat the exact task or workflow ID with --confirm.', EXIT.usage, 'confirmation_required');
        ...
        if (action === 'export') {
            ...
            const paths = [];
            while (args.includes('--path'))
                paths.push(option(args, '--path'));
            if (paths.length > 20)
                throw new CliError('Review at most 20 paths at a time.', EXIT.usage, 'selection_invalid');
            ...
            if (confirmation !== id)
                throw new CliError(`Repeat the task ID with --confirm to ${action} this task.`, EXIT.usage, 'confirmation_required');
            if (action === 'approve' || action === 'rollback-approve') {
                const value = option(args, '--digest');
                if (!value || !digest.test(value))
                    throw new CliError('Pass the action_digest from the review you inspected.', EXIT.usage, 'review_digest_required');
                body = { action_digest: value };
    ...
    if (moduleBuildExpected)
        validateModuleBuild(result, moduleBuildExpected, action);
    if (starterExpected)
        validateStarter(result, starterExpected, action);
    if (action === 'resume') {
    ...
    if (action === 'compatibility')
        validateCompatibility(result, path.split('/')[4]);
    if (action === 'sources')
        validateResearchCatalog(result, path.split('/')[4]);
    ...
    if (action === 'events' && result && typeof result === 'object') {
        return { message: 'Task activity loaded.', data: result.events ?? [] };
    }
